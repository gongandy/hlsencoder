...
+ (KFAPIClient*) sharedClient {
    ...
    dispatch_once(&onceToken, ^{ //makes sure only happens once
...
- (void) requestNewActiveUserWithUsername:(NSString *)username password:(NSString *)password email:(NSString *)email displayName:(NSString *)displayName extraInfo:(NSDictionary *)extraInfo callbackBlock:(void (^)(KFUser *, NSError *))callbackBlock {
    ...
    [parameters setObject:@"12345" forKey:@"uuid"];
    
    [parameters setObject:@"crowdseye" forKey:@"appName"];
    
    KFUser *user = [MTLJSONAdapter modelOfClass:[KFUser class] fromJSONDictionary:parameters error:nil];
...
- (void) startStreamWithParameters:(NSDictionary*)parameters callbackBlock:(void (^)(KFStream *, NSError *))endpointCallback {
    ...
    //coming in with no parameters
//    [parameters setValue:@"HLS" forKey:@"streamType"];
//    [parameters setValue:@"streamID" forKey:@"12345"];
//    [parameters setValue:@"startDate" forKey:[NSDate date]];
//    KFStream *endpoint = [MTLJSONAdapter modelOfClass:[KFS3Stream class] fromJSONDictionary:parameters error:nil];
    KFS3Stream *endpoint = [[KFS3Stream alloc] init];
    endpoint.streamType = @"HLS";
    endpoint.streamID = @"12345";
    endpoint.startDate = [NSDate date];
    endpoint.username = [[KFUser activeUser] username];
